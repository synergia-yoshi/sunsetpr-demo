...
2. The installed SunsetPR App created [draft repair PR #2](https://github.com/synergia-yoshi/sunsetpr-demo/pull/2), changed five model references, added generated migration guards, and left the environment-backed value for runtime confirmation.

The draft repair PR's repository-side CI runs type checking, lint, existing tests, and the generated compatibility test. SunsetPR does not merge the PR.

The free workflow is pinned to the immutable [v0.2.0](https://github.com/synergia-yoshi/sunsetpr-action/releases/tag/v0.2.0), whose bundled database contains 105 model IDs and two API shutdown surfaces checked against provider documentation on 2026-07-19.
